...
```{r}
cob_doce %>% 
  filter(metodo %in% c("point_quadrat", "quadrat medio")) %>% 
  correla_pares("quadrat medio", 
                xlab = "Cobertura (%) Point Quadrat", 
                ylab = "Cobertura (%) Quadrat medio") +
  xlim(0,80) + ylim(0,80) + 
  scale_shape_manual(values = 15, name = "") +
...
riq_selected <- riq %>% filter(metodo %in% c("line_intercept","point_quadrat","quadrat_parcela")) 
```
...
Tal y como observamos en la Figura \@ref(fig:riq-correla), los métodos de **line intercept** y **point quadrat**, subestiman la riqueza estimada por el método **quadrat parcela**.  

```{r riq-correla, fig.cap="Correlación entre los valores de riqueza estimados por Line Intercept y los otros métodos de campo: Point Quadrat y Quadrat Parcela"}

riq_selected %>% 
  dplyr::select(parcela, value, metodo) %>% 
  correla_pares("quadrat_parcela", 
                xlab = "Riqueza", 
                ylab = "Riqueza Quadrat Parcela") + 
  xlim(0,60) + ylim(0,60) + 
...
riq_selected %>% 
  dplyr::select(parcela, value, metodo) %>% 
  filter(metodo %in% c("line_intercept","point_quadrat")) %>%  
  correla_pares("line_intercept", 
                xlab = "Riqueza Point Quadrat", 
                ylab = "Riqueza Line intercept") + 
 #  xlim(5,25) + ylim(5,25) + 
  scale_shape_manual(values = 15, name = "") +
  scale_colour_manual(values = "black", name = "") + 
  scale_fill_manual(values = "black", name = "") +
...
El método **line intercept** subestima la diversidad en comparación con el método **point quadrat** (Figura \@ref(fig:div-correla)), aunque presentan una significativa y alta correlación ($R^2=$ 0.82); mientras que el LI sobreestima la diversidad en comparación con el método **quadrat medio**. 
...
div_selected %>% 
  ...
  correla_pares("line_intercept", 
                xlab = "Diversidad", 
                ylab = "Diversidad Line Intercept") +
...
div_selected %>% 
  dplyr::select(parcela, value, metodo) %>% 
  filter(metodo %in% c("quadrat medio","point_quadrat")) %>% 
  correla_pares("quadrat medio", 
                xlab = "Diversidad Point Quadrat", 
                ylab = "Diversidad Quadrat medio") + 
  xlim(.5,3) + ylim(.5,3) + 
  scale_shape_manual(values = 15, name = "") +
  scale_colour_manual(values = "black", name = "") + 
  scale_fill_manual(values = "black", name = "") +
