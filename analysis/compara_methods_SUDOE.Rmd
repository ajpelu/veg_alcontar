...
ct <- partykit::ctree(dronQ ~ quadrat, data = dfcob_dronq)
ct
sctest(ct) 
```
...
```{r cobdronq-error5}
set.seed(123)
ctrpart <- rpart(dronQ ~ quadrat, data = dfcob_dronq)
printcp(ctrpart) %>% 
...
```{r}
set.seed(123)
ctrpart2 <- rpart(dronQ ~ quadrat, data = dfcob_dronq, control = 
...
set.seed(123)
ct_ok <- partykit::ctree(dronQ ~ quadrat, data = dfcob_dronq,
...
fit_q %>% dplyr::select(id = quadrat, plot, dronQ=dron) %>%
  rename(parcela = id, quadrat = plot) %>% 
  pivot_longer(cols = c("dronQ", "quadrat")) %>% 
  rename(metodo = name) %>% 
  correla_pares("dronQ", 
